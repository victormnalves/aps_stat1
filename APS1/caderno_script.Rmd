...
## Segregando a contagem dos resultados por estação por ano nos estados 'quentes'
```{r}
...
## Segregando a contagem dos resultados por estação por ano nos estados 'não quentes'
```{r}
...
## Gols nos estados por estação por ano
```{r}
...
## Calculando medidas dos gols da casa por ano
```{r}
medidas_gols_ano <- as.tibble(dados %>% group_by(temporada, res)%>%
                                  filter(res == 'C') %>% 
                                  summarise('media' = mean(golcasa),
                                            'mediana' = median(golcasa),
                                            'desvpad' = sd(golcasa)))

medidas_gols_estado <- as.tibble(dados %>% group_by(estado) %>%
                                  filter(res == 'C') %>% 
                                  summarise('media' = mean(golcasa),
                                            'mediana' = median(golcasa),
                                            'desvpad' = sd(golcasa)))
...
```
## Plot de gols por estado
```{r}
dist_med_gols_estado <- medidas_gols_estado %>% mutate(estado = fct_reorder(estado, media)) %>%
...
```{r}
medidas_gols_ano %>%
  ggplot(aes(media, temporada, colour = media)) + geom_point() +
  scale_color_viridis(discrete = FALSE) +
