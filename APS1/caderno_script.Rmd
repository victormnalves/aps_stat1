...
Observando o aproveitamento do mandante
```{r}
cont_res_casa <-  as_tibble(dados %>% group_by(temporada, casa, res) %>% filter(casa %in% c('Corinthians', 'Flamengo', 'Sao Paulo', 'Palmeiras', 'Fluminense') & temporada %in% c('2016','2017','2018', '2019', '2020')) %>% summarise(freq = n()) %>% mutate(freq_rel = freq/sum(freq)))
```
...
dist_sg_per <- dados %>% group_by(periodo) %>%
  ...
  geom_histogram(alpha = 0.8, bins = 10, breaks = seq(0,10, 1), position = 'identity') + 
  xlim(0,8) + 
  scale_fill_viridis(discrete = TRUE) + 
...
dist_sg_al <- dados %>% filter(estado == 'Alagoas') %>%
  ...
  geom_histogram(alpha = .8 ,color = 'black', 
                 breaks = seq(0,10, 1), position = 'identity') + 
  xlim(0, 4) +
  scale_fill_viridis(discrete = TRUE) +
