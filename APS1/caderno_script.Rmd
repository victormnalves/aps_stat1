...
soma_estado_inverno2012 <- dados %>% filter(data %in% inverno_2012$data) %>% group_by(estado) %>%
                            mutate(somaestado = sum(somagols)) %>% select(estado,temporada, somaestado)
...
dist_sg_per <- dados %>% group_by(periodo) %>%
  ggplot(aes(somagols,stat(density) ,fill = periodo)) + 
...
```{r}
somagols_inverno2012 <- medidas_sg_inverno_2012 %>% mutate(estado = fct_reorder(estado, media)) %>% 
  group_by(estado) %>%
...
somagols_verao2012 <- medidas_sg_verao_2012 %>% mutate(estado = fct_reorder(estado, media)) %>% 
  group_by(estado) %>%
  ggplot(aes(media, estado, colour = periodo)) + geom_point() +
  scale_color_viridis(discrete = TRUE) +
  ggtitle('Média de gols no frio por período por estado em 2012') + 
...
somagols_inverno2013 <- medidas_sg_inverno_2013 %>% mutate(estado = fct_reorder(estado, media)) %>% 
  group_by(estado) %>%
  ggplot(aes(media, estado, colour = periodo)) + geom_point() +
  scale_color_viridis(discrete = TRUE) +
...
somagols_verao2013 <- medidas_sg_verao_2013 %>% mutate(estado = fct_reorder(estado, media)) %>% 
  group_by(estado) %>%
  ggplot(aes(media, estado, colour = periodo)) + geom_point() +
  scale_color_viridis(discrete = TRUE) +
  ggtitle('Média de gols no frio por período por estado em 2013') + 
...
Unindo uniões
```{r}
gols20122014 <- grid.arrange(arrangeGrob(somagols_verao2012, somagols_inverno2012,
                         somagols_verao2014, somagols_inverno2014), 
                         top = 'Distribuição da média da soma dos gols por clima em 2012 e 2014')

gols20162018 <- grid.arrange(arrangeGrob(somagols_verao2016, somagols_inverno2016,
                         somagols_verao2018, somagols_inverno2018), 
