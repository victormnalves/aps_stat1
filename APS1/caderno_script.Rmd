...
## Trabalhando com a soma dos gols por período do dia e por temporada
```{r}
...
Calculando medidas dos gols por ano

```{r}
somagols_ano <- dados %>% group_by(golcasa, temporada, res)


medidas_gols_ano <- as.tibble(somagols_ano %>% group_by(temporada)%>%
                                  filter(res == 'C') %>% 
                                  summarise('media' = mean(golcasa),
                                            'mediana' = median(golcasa),
                                            'desvpad' = sd(golcasa)))

medidas_gols_noite <- as.tibble(somagols_estado_periodo_ano %>% group_by(temporada) %>%
                                  filter(periodo == 'Noite') %>% 
                                  summarise('media' = mean(n),
                                            'mediana' = median(n),
                                            'desvpad' = sd(n)))
...
```{r}
medidas_gols_ano %>% 
  ggplot(aes(temporada, media, colour = temporada)) + geom_point() +
  scale_color_viridis(discrete = FALSE) +
  ggtitle('Media de gols a tarde por ano') + 
...
dist_frio_gelados2012 <- climao_gelado %>% 
  filter(data > '2012-03-20' & data < '2012-09-22') %>%
...
dist_calor_gelados2012 <- climao_gelado %>% 
  filter(data > '2012-09-22' & data < '2013-03-20') %>%
  ggplot(aes(n, fill = res)) + 
  geom_bar(alpha = .6,color = 'black') + 
  scale_fill_brewer(palette = 'Dark2') +
...
dist_frio_gelados2013 <- climao_gelado %>% 
  filter(data > '2013-03-20' & data < '2013-09-22') %>%
  ggplot(aes(n, fill = res)) + 
  geom_bar(alpha = .6,color = 'black') + 
  scale_fill_brewer(palette = 'Dark2') +
...
dist_calor_gelados2013 <- climao_gelado %>% 
  filter(data > '2013-09-22' & data < '2014-03-20') %>%
  ggplot(aes(n, fill = res)) + 
  geom_bar(alpha = .6,color = 'black') + 
  scale_fill_brewer(palette = 'Dark2') +
...
dist_frio_quentes2012 <- climao_quente %>% 
  filter(data > '2013-03-20' & data < '2012-09-22') %>%
  ggplot(aes(n, fill = res)) + 
  geom_bar(alpha = .6,color = 'black') + 
  scale_fill_brewer(palette = 'Dark2') +
